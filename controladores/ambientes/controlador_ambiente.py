...
def eliminar_ambiente(idambiente):
    ...
    try:
        with conexion.cursor() as cursor:
            cursor.execute("delete from ambiente where idambiente = %s)",
                           (idambiente))
            conexion.commit()
...
def modificar_ambiente(nombre, aforo, estado, idedificio, idambientetipo):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("update from ambiente (nombre, aforo, estado, idedificio, idambientetipo) VALUES (%s, %s, %s, %s, %s))",
                           (nombre, aforo, estado, idedificio, idambientetipo))
            conexion.commit()
